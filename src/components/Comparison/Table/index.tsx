export default function Table() {
  return (
    <div className="mt-[-40px]">
      <div className="w-screen overflow-auto tablet:w-full">
        <table>
          <thead className="border-b border-b-white ">
            <tr>
              <th className="px-[20px] py-[27.5px] text-start font-unbound text-[16px] font-bold leading-[1.2] tablet:text-[22px] desktop:text-[28px]">
                Designing
              </th>
              <th className="px-[20px] py-[27.5px] text-start font-proxima text-[18px] font-bold leading-[1.2] tablet:text-[20px] desktop:text-[36px]">
                Freelancers/Indians
              </th>
              <th className="px-[20px] py-[27.5px] text-start font-proxima text-[18px] font-bold leading-[1.2] text-[#013EDF] tablet:text-[20px] desktop:text-[36px]">
                Bright Byte
              </th>
              <th className="px-[20px] py-[27.5px] text-start font-proxima text-[18px] font-bold leading-[1.2] tablet:text-[20px] desktop:text-[36px]">
                Development Studio
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="w-[25%] p-[20px] pt-[30px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:pt-[50px] tablet:text-[16px] desktop:pt-[60px] desktop:text-[20px]">
                Requirement Gathering
              </td>
              <td className="w-[25%] p-[20px] pt-[30px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:pt-[50px] tablet:text-[16px] desktop:pt-[60px] desktop:text-[20px]">
                Superficial
              </td>
              <td className="w-[25%] p-[20px] pt-[30px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:pt-[50px] tablet:text-[16px] desktop:pt-[60px] desktop:text-[20px]">
                Flexible and Personalized
              </td>
              <td className="w-[25%] p-[20px] pt-[30px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:pt-[50px] tablet:text-[16px] desktop:pt-[60px] desktop:text-[20px]">
                Standardized, Long and Expensive
              </td>
            </tr>
            <tr className="bg-[#F2F5FF1A]/10">
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Creating Technical Task
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Incomplete
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Detailed
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Detailed
              </td>
            </tr>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Level of Consulting
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Low
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Best practices from different spheres
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Depends on the client
              </td>
            </tr>
            <tr className="bg-[#F2F5FF1A]/10">
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Interface Design
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Mediocre UX/UI
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Best practices from different spheres
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Average quality
              </td>
            </tr>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Visual Design
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Not Trendy
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Innovative
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Stable
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <div className="w-screen overflow-auto tablet:w-full">
        <table className="w-full">
          <thead className="border-b border-b-white ">
            <tr>
              <th className="px-[20px] py-[27.5px] pt-[67.5px] text-start font-unbound text-[16px] font-bold leading-[1.2] tablet:text-[22px] desktop:text-[28px]">
                Development
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Code Writing
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Fast
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Quality and Fast
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Quality and Fast
              </td>
            </tr>
            <tr className="bg-[#F2F5FF1A]/10">
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Integration
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Impossible
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Optimal integration considering <br />
                client&apos;s specifics
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Expensive integration
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <div className="w-screen overflow-auto tablet:w-full">
        <table className="w-full">
          <thead className="border-b border-b-white ">
            <tr>
              <th className="px-[20px] py-[27.5px] pt-[67.5px] text-start font-unbound text-[16px] font-bold leading-[1.2] tablet:text-[22px] desktop:text-[28px]">
                Testing
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Unit Tests
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Absent
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Full testing at all levels
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Full testing at all levels
              </td>
            </tr>
            <tr className="bg-[#F2F5FF1A]/10">
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Auto Testing
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Impossible
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Full testing at all levels
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Full testing at all levels
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <div className="w-screen overflow-auto tablet:w-full">
        <table className="w-full">
          <thead className="border-b border-b-white ">
            <tr>
              <th className="px-[20px] py-[27.5px] pt-[67.5px] text-start font-unbound text-[16px] font-bold leading-[1.2] tablet:text-[22px] desktop:text-[28px]">
                Support and Maintenance
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Technical Support
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Depends on availability
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Dedicated Support
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Support after signing
              </td>
            </tr>
            <tr className="bg-[#F2F5FF1A]/10">
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Updates and Upgrades
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Irregular
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Regular and on client&apos;s request
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Constant and expensive
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <div className="w-screen overflow-auto tablet:w-full">
        <table className="w-full">
          <thead className="border-b border-b-white ">
            <tr>
              <th className="px-[20px] py-[27.5px] pt-[67.5px] text-start font-unbound text-[16px] font-bold leading-[1.2] tablet:text-[22px] desktop:text-[28px]">
                Project Management
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Planning and Coordination
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Minimal Project Management
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Management with risk management consideration
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Bureaucratic and slow
              </td>
            </tr>
            <tr className="bg-[#F2F5FF1A]/10">
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Documentation Management
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Absent
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Full and Current Documentation
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Full and Current Documentation
              </td>
            </tr>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Communication with Client
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Inconsistent and Superficial
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Regular and Personalized
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Regular and Personalized
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <div className="w-screen overflow-auto tablet:w-full">
        <table className="w-full">
          <thead className="border-b border-b-white ">
            <tr>
              <th className="px-[20px] py-[27.5px] pt-[67.5px] text-start font-unbound text-[16px] font-bold leading-[1.2] tablet:text-[22px] desktop:text-[28px]">
                Total
              </th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Development Quality
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Low
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                High
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                High
              </td>
            </tr>
            <tr className="bg-[#F2F5FF1A]/10">
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Support
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Absent
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Available
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Available
              </td>
            </tr>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Product Team Cost / month
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Up to 5k$
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Up to 20k$
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Up to 50k$
              </td>
            </tr>
            <tr className="bg-[#F2F5FF1A]/10">
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Mobile Team / month
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Up to 3k$
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Up to 12k$
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Up to 30k$
              </td>
            </tr>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Consulting
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Absent
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                Best practices
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]">
                Within the industry
              </td>
            </tr>
            <tr>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-bold leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]"></td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]"></td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white tablet:text-[16px] desktop:text-[20px]">
                <span className="font-proxima text-[18px] font-bold leading-[1.2] text-[#013EDF] tablet:text-[20px] desktop:text-[36px]">
                  Clients Choice
                </span>
              </td>
              <td className="w-[25%] p-[20px] font-proxima text-[12px] font-normal leading-[1.2] text-white/60 tablet:text-[16px] desktop:text-[20px]"></td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
  );
}
